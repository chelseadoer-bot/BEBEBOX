...
function openAiApp(slug,label){
  ...
  url+=(url.indexOf("?")<0?"?":"&")+"_v=8";   // 앱 정적파일 캐시 무력화
...
function maybeWelcomeBonus(){
  ...
  // 초대·게스트로 '남의 가족'에 합류했거나, 이미 기록이 있는 가족이면 환영 첫 기록을 심지 않는다.
  // (남의 피드에 '우리 아기 첫 기록' 인증서가 생기고 +20캔디가 지급되던 문제 방지)
  const joinedExisting=(typeof isGuest==="function"&&isGuest())
    ||(Array.isArray(state.posts)&&state.posts.length>0);
  if(joinedExisting){
    localStorage.setItem("bb_welcome_v1","1");   // 다음 입장에서 재시도 안 함
    return;
  }
